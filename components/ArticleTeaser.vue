...
const formattedDate = useDateFormat(new Date(props.post.created), "D. M. YYYY");
</script>

<template>
  <div class="py-8 sm:py-12">
    <a v-if="post?.fields?.image" :href="post?._path" class="group relative block">
      <NuxtPicture width="1200" height="675" :src="post.fields?.image" :alt="post.fields?.imageAlt" format="webp"
        loading="lazy" />
      <div
      ...
      <div
        ...
        <span>Read More</span>
        <Icon class="mt-1 h-5 w-5 text-blue-400" aria-hidden="true" name="majesticons:fast-forward-line" />
      </div>
    </a>
    ...
    <a v-if="post.title || post.description" :href="post._path" class="group mt-4 block text-blue-700 dark:text-blue-300">
      <h2 class="text-xl font-bold underline lg:text-2xl lg:no-underline lg:group-hover:underline">
        {{ post.title }}
      </h2>
    ...
    <div class="mt-6 flex items-center">
      <div class="flex space-x-1 text-sm">
        <time :datetime="props.post.created">
          {{ formattedDate }}
        </time>
